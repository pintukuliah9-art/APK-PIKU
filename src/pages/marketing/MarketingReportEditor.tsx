...
import { SavedMarketingReport } from '../../types/app';
import { Save, Download, ArrowLeft, CheckCircle } from 'lucide-react';
// @ts-ignore
import html2pdf from 'html2pdf.js';
import { AlertModal } from '../../components/AlertModal';

export default function MarketingReportEditor() {
  const { id } = useParams();
  const navigate = useNavigate();
  const { marketingReports, addMarketingReport, updateMarketingReport, settings } = useAppStore();
  const [data, setData] = useState<MarketingReportData>(initialMarketingReportData);
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      const opt = {
        margin:       15,
        filename:     `Laporan_Marketing_${data.month}_${data.year}.pdf`,
  ...
  useEffect(() => {
    if (id) {
      const existingReport = marketingReports.find(r => r.id === id);
      if (existingReport) {
  ...
  }, [id, marketingReports, settings]);

  const handleSave = async (status: 'draft' | 'final' = 'draft') => {
    setIsSaving(true);
    try {
      if (id) {
        updateMarketingReport(id, { ...data, status });
        setAlertModal({
          isOpen: true,
          title: 'Berhasil',
        ...
        const newId = addMarketingReport({ ...data, status });
        navigate(`/internal/marketing/edit/${newId}`, { replace: true });
        setAlertModal({
          isOpen: true,
          title: 'Berhasil',
    ...
    <div className="max-w-7xl mx-auto space-y-6 pb-12">
      <div className="flex items-center justify-between border-b border-blue-800 pb-6">
        <div className="flex items-center gap-4">
          <button
            onClick={() => navigate('/internal/marketing/reports')}
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">
              {id ? 'Edit Laporan Marketing' : 'Buat Laporan Marketing'}
            </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
          ...
          <div className="p-6 overflow-y-auto max-h-[calc(100vh-250px)] custom-scrollbar">
            <MarketingReportForm data={data} onChange={setData} />
          </div>
        </div>
        ...
        <div className="bg-gray-100 rounded-2xl shadow-inner border border-gray-200 dark:border-slate-700 overflow-hidden flex flex-col">
          ...
          <div className="p-8 overflow-y-auto max-h-[calc(100vh-250px)] custom-scrollbar flex-1 bg-gray-100">
            <div ref={componentRef} className="flex flex-col gap-8 items-center">
              <MarketingReportPreview data={data} />
            </div>
          </div>
        </div>
      </div>
